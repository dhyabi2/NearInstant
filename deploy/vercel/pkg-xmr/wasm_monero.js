 ...
 * @returns {string}
 */
export function xmr_joint_info(ctx, spend_pub_a, spend_pub_b, view_half_a, view_half_b, network) {
    let deferred8_0;
    let deferred8_1;
    try {
 ...
 * @returns {Uint8Array}
 */
export function xmr_joint_secret(ctx, my_secret, their_secret) {
    const ptr0 = passArray8ToWasm0(ctx, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
 ...
 * @returns {string}
 */
export function xmr_personal(seed, network) {
    let deferred4_0;
    let deferred4_1;
    try {
 ...
 * @returns {Uint8Array}
 */
export function xmr_spend_pub(secret) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('wasm_monero_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
